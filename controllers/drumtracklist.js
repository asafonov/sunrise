class DrumTrackListController {
  ...
  onIsPlayingUpdate ({isPlaying, repeat}) {
    if (this.timeout) {
    ...
    this.timeout = setTimeout(() => this.onIsPlayingUpdate({isPlaying, repeat: true}), this.getInterval() * 16)

    if (repeat) return asafonov.waveUtils.play()
